...
import { Plus, Check, Upload, Trash2 } from 'lucide-react';
...
export default function RoleplayCatalog({ papeis, onClose, onPapeisChanged }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const publicUrl = await uploadImage(file, 'papeis', profile.id);
      if (publicUrl) {
        setAvatarUrl(publicUrl);
      } else {
        alert("Aviso de Upload: Verifique se o bucket 'imagens' existe no Supabase");
      }
    } catch (err) {
      console.error(err);
    } finally {
      ...
      {papeis.length === 0 && !showCreate ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {loading ? <LoadingSpinner size={18} className="text-white" /> : <Check size={18} />}
              Criar
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          {papeis.map((papel) => (
            <button
              ...
              style={{ backgroundColor: sanitizeHexColor(papel.cor_balao, '#8A2BE2') + '20' }}
            >
              <PapelAvatar papel={papel} size="md" />
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{
                backgroundColor: sanitizeHexColor(selectedPapel.cor_balao, '#8A2BE2'),
                color: sanitizeHexColor(selectedPapel.cor_fonte, '#FFFFFF'),
              }}
            >
